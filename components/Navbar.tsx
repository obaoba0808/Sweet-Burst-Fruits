...
const Navbar: React.FC<NavbarProps> = ({ onNavClick, cartCount, onOpenCart }) => {
  ...
  const textColorClass = (scrolled || mobileMenuOpen) ? 'text-[#1A2E1A]' : 'text-white';
      ...
      >
        <div className="max-w-[1800px] mx-auto px-8 flex items-center justify-between">
          ...
          <div className={`hidden md:flex items-center gap-12 text-sm font-semibold tracking-widest uppercase transition-colors duration-500 ${textColorClass}`}>
            <a href="#products" onClick={(e) => handleLinkClick(e, 'products')} className="hover:text-[#D97706] transition-colors">鮮果商城</a>
            <a href="#about" onClick={(e) => handleLinkClick(e, 'about')} className="hover:text-[#D97706] transition-colors">關於我們</a>
            <a href="#journal" onClick={(e) => handleLinkClick(e, 'journal')} className="hover:text-[#D97706] transition-colors">美味誌</a>
          </div>
